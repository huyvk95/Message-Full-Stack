...
async function get(request: any) {
    ...
    let result: any[] = [];
    // Find
    await User.find({ email: { "$regex": `.*${string}*`, "$options": "i" }, active: true, _id:{"$ne": request.socket.authToken?._id} }).limit(50)
        .select(common.dbselect.user)
        .then(data => { result = result.concat(data) })
    await User.find({ firstName: { "$regex": `.*${string}*`, "$options": "i" }, active: true, _id:{"$ne": request.socket.authToken?._id} }).limit(50)
        .select(common.dbselect.user)
        .then(data => { result = result.concat(data) });
    await User.find({ lastName: { "$regex": `.*${string}*`, "$options": "i" }, active: true, _id:{"$ne": request.socket.authToken?._id} }).limit(50)
        .select(common.dbselect.user)
        .then(data => { result = result.concat(data) });
    // Data handle
